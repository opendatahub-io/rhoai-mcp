...
class TestPortForwardDiscovery:
    """Tests for port-forward discovery when running outside the cluster."""
    ...
    def test_discover_returns_internal_url(
        self, mock_k8s: MagicMock, discovery: ModelRegistryDiscovery
    ) -> None:
        """Test that discover() returns internal service URL.

        The sync discover() method always returns the internal URL.
        Port-forwarding is handled by the async discover_with_port_forward() method.
        """
        # Setup CRD response
        mock_k8s.list_resources.return_value = [
            self._make_mock_component("rhoai-model-registries")
        ...
        assert result is not None
        # discover() always returns internal URL
        assert result.url == "http://model-catalog.rhoai-model-registries.svc:8080"
        assert result.is_external is False
        assert result.source == "crd"

    def test_discover_uses_internal_url_regardless_of_cluster_location(
        self, mock_k8s: MagicMock, discovery: ModelRegistryDiscovery, monkeypatch: Any
    ) -> None:
        """Test that internal URL is returned even when outside cluster.

        The sync discover() method does not attempt port-forwarding.
        Port-forward is handled by discover_with_port_forward().
        """
        # Mock running outside cluster - should not affect sync discover()
        monkeypatch.setattr(
        ...
        assert result is not None
        # Always returns internal URL (sync discover doesn't do port-forward)
        assert result.url == "http://model-catalog.rhoai-model-registries.svc:8080"
        assert result.is_external is False
        assert result.source == "crd"
