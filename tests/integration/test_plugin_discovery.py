...
def test_plugin_manager_loads_core_plugins():
    ...
    # Should load 9 core domain plugins + 4 composite plugins (incl. neuralnav) = 13 total
    assert count == 13
    ...
    # Composite plugins (4, incl. neuralnav)
    expected_composites = {
        "cluster-composites",
        "training-composites",
        "neuralnav",
...
def test_get_composite_plugins_returns_plugin_instances():
    ...
    assert len(plugins) == 4  # cluster, training, neuralnav, meta composites
